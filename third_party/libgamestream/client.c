...
//
// Redefinir printf solo para PSVita, para Windows usar printf normal
#ifdef __vita__
...
#define UNIQUE_ID_UUID_STRLEN 37
static char unique_id_uuid[UNIQUE_ID_UUID_STRLEN];
static X509 *cert;
...
// --- UUID universal portable ---
static void gen_uuid_v4(unsigned char out[16]) {
    for (int i = 0; i < 16; ++i) {
...
static void uuid_to_str(const unsigned char in[16], char out[UUID_STRLEN]) {
    sprintf(out,
...
static int load_unique_id(const char* keyDirectory) {
  ...
  // Convertir unique_id (hex plano) a formato UUID estándar con guiones
  unsigned char unique_id_bytes[16];
  for (int i = 0; i < 16; ++i) {
    sscanf(&unique_id[i*2], "%2hhx", &unique_id_bytes[i]);
  }
  uuid_to_str(unique_id_bytes, unique_id_uuid);
...
static int load_serverinfo(PSERVER_DATA server, bool https) {
  ...
  gen_uuid_v4(uuid);
  printf("[UUID][CLIENT] gen_uuid_v4 (en client.c): ");
  for (int i = 0; i < 16; ++i) printf("%02X", uuid[i]);
  printf("\n");
  uuid_to_str(uuid, uuid_str);
  printf("[UUID][CLIENT] uuid_to_str (en client.c): %s\n", uuid_str);

  snprintf(url, sizeof(url), "%s://%s:%d/serverinfo?uniqueid=%s&uuid=%s",
    https ? "https" : "http", server->serverInfo.address, https ? server->httpsPort : server->httpPort, unique_id_uuid, uuid_str);
...
int gs_unpair(PSERVER_DATA server) {
  ...
  gen_uuid_v4(uuid);
  uuid_to_str(uuid, uuid_str);
  snprintf(url, sizeof(url), "http://%s:%u/unpair?uniqueid=%s&uuid=%s", server->serverInfo.address, server->httpPort, unique_id_uuid, uuid_str);
...
int gs_pair(PSERVER_DATA server, char* pin) {
  ...
  if (RAND_bytes(salt_data, sizeof(salt_data)) != 1) {
    // Fallback a rand() si OpenSSL falla
    for (int i = 0; i < sizeof(salt_data); ++i) salt_data[i] = rand() & 0xFF;
  }
  bytes_to_hex(salt_data, salt_hex, sizeof(salt_data));

  gen_uuid_v4(uuid);
  uuid_to_str(uuid, uuid_str);
  snprintf(url, sizeof(url), "http://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=roth&updateState=1&phrase=getservercert&salt=%s&clientcert=%s", server->serverInfo.address, server->httpPort, unique_id_uuid, uuid_str, salt_hex, cert_hex);
  PHTTP_DATA data = http_create_data();
  if (data == NULL)
    return GS_OUT_OF_MEMORY;
  ...
  if (RAND_bytes(challenge_data, sizeof(challenge_data)) != 1) {
    for (int i = 0; i < sizeof(challenge_data); ++i) challenge_data[i] = rand() & 0xFF;
  }
  ...
  gen_uuid_v4(uuid);
  uuid_to_str(uuid, uuid_str);
  snprintf(url, sizeof(url), "http://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=roth&updateState=1&clientchallenge=%s", server->serverInfo.address, server->httpPort, unique_id_uuid, uuid_str, challenge_hex);
  ...
  if (RAND_bytes((unsigned char*)client_secret_data, sizeof(client_secret_data)) != 1) {
    for (int i = 0; i < sizeof(client_secret_data); ++i) client_secret_data[i] = rand() & 0xFF;
  }
  ...
  gen_uuid_v4(uuid);
  uuid_to_str(uuid, uuid_str);
  snprintf(url, sizeof(url), "http://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=roth&updateState=1&serverchallengeresp=%s", server->serverInfo.address, server->httpPort, unique_id_uuid, uuid_str, challenge_response_hex);
  if ((ret = http_request(url, data)) != GS_OK)
    goto cleanup;
  ...
  gen_uuid_v4(uuid);
  uuid_to_str(uuid, uuid_str);
  snprintf(url, sizeof(url), "http://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=roth&updateState=1&clientpairingsecret=%s", server->serverInfo.address, server->httpPort, unique_id_uuid, uuid_str, client_pairing_secret_hex);
  if ((ret = http_request(url, data)) != GS_OK)
    goto cleanup;
  ...
  gen_uuid_v4(uuid);
  uuid_to_str(uuid, uuid_str);
  snprintf(url, sizeof(url), "https://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=roth&updateState=1&phrase=pairchallenge", server->serverInfo.address, server->httpsPort, unique_id_uuid, uuid_str);
  if ((ret = http_request(url, data)) != GS_OK)
    goto cleanup;
...
int gs_applist(PSERVER_DATA server, PAPP_LIST *list) {
  ...
  gen_uuid_v4(uuid);
  uuid_to_str(uuid, uuid_str);
  snprintf(url, sizeof(url), "https://%s:%u/applist?uniqueid=%s&uuid=%s", server->serverInfo.address, server->httpsPort, unique_id_uuid, uuid_str);
  if (http_request(url, data) != GS_OK)
    ret = GS_IO_ERROR;
...
int gs_start_app(PSERVER_DATA server, STREAM_CONFIGURATION *config, int appId, bool sops, bool localaudio, int gamepad_mask) {
  ...
  char url[4096];
  uint32_t rikeyid = 0;
  if (RAND_bytes(config->remoteInputAesIv, sizeof(rikeyid)) != 1) {
    for (int i = 0; i < sizeof(rikeyid); ++i) ((unsigned char*)config->remoteInputAesIv)[i] = rand() & 0xFF;
  }
  ...
  snprintf(url, sizeof(url), "https://%s:%u/%s?uniqueid=%s&uuid=%s&appid=%d&mode=%dx%dx%d&additionalStates=1&sops=%d&rikey=%s&rikeyid=%d&localAudioPlayMode=%d&surroundAudioInfo=%d&remoteControllersBitmap=%d&gcmap=%d%s%s",
           server->serverInfo.address, server->httpsPort, server->currentGame ? "resume" : "launch", unique_id_uuid, uuid_str, appId, config->width, config->height, fps, sops, rikey_hex, rikeyid, localaudio, surround_info, gamepad_mask, gamepad_mask,
...
int gs_quit_app(PSERVER_DATA server) {
  ...
  gen_uuid_v4(uuid);
  uuid_to_str(uuid, uuid_str);
  snprintf(url, sizeof(url), "https://%s:%u/cancel?uniqueid=%s&uuid=%s", server->serverInfo.address, server->httpsPort, unique_id_uuid, uuid_str);
  if ((ret = http_request(url, data)) != GS_OK)
    goto cleanup;
