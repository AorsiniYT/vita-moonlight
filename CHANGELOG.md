...
- Rebuilt the client around the Borealis interface with native controller and touch navigation.
- Added low-latency Vita hardware decoding through both the legacy and FFmpeg-vita paths.
- Added direct GXM video presentation, decoder low-delay mode and stale-frame recovery to keep playback close to real time.
- Added configurable stream resolutions from 848x480 through 1366x768, automatic bitrate selection, aspect-ratio controls and live display settings.
- Added host discovery, pairing, editing, Wake-on-LAN and automatic stream reconnection after resuming the application.
- Added DS4 touchpad, absolute mouse and Sunshine tablet modes, configurable front and rear touch zones, gyroscope controls and user-defined shortcuts.
- Added legacy and modern virtual keyboards with configurable layouts.
- Added Moonmic microphone forwarding, host monitor resolution control and microphone diagnostics.
- Added English, Spanish, French, Japanese, Brazilian Portuguese, Russian and Traditional Chinese interfaces.

### 🔧 Core Improvements

- Reduced video latency and micro-stuttering with direct frame submission, frame pacing, startup resynchronization and latest-frame presentation.
- Restored local audio handling and improved Opus playback, variable frame sizes and stream audio lifecycle management.
- Added an in-stream settings overlay with adjustable background opacity, brightness control and an option to prevent screen dimming.
- Fixed touchscreen input across the supported input modes and separated active touch zones from normal touch input.
- Added a reconnecting overlay and controller-state recovery when returning from suspend.
- Added asynchronous logging and network diagnostics without blocking video decoding.
- Limited encoded stream resolutions to modes the Vita AVC decoder can sustain without accumulating delay.

### ⚙️ Under the Hood

- Added a shared build flow for Linux, WSL and Windows/MSYS2.
- Added project dependency installation through VitaSDK packages and the pinned FFmpeg-vita hardware-decoder fork.
- Moved the Borealis Vita changes into a CMake-applied patch instead of modifying the submodule.
- Separated visible video dimensions from aligned decoder storage to handle non-native resolutions safely.
- Removed obsolete platform code, duplicate build files and unused dependencies.
